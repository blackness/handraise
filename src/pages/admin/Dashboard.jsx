...
export function AdminDashboard() {
  ...
  async function loadInstitution() {
    const { data } = await supabase
      .from('admin_profiles')
      .select('institution_id, institutions(id, name, plan, status)')
      .eq('user_id', user.id)
      .single()
  ...
  async function loadStats() {
    ...
    if (!ap) return
    const id = ap.institution_id
    const [s, p, sess] = await Promise.all([
      supabase.from('student_profiles').select('id', { count: 'exact', head: true }).eq('institution_id', id).eq('active', true),
      supabase.from('programs').select('id', { count: 'exact', head: true }).eq('institution_id', id).eq('archived', false),
      supabase.from('sessions').select('id', { count: 'exact', head: true }).eq('institution_id', id).eq('status', 'ended'),
    ])
  ...
  async function loadSessions() {
    ...
    const { data: active } = await supabase
      .from('sessions')
      .select('id, status, started_at, programs(name), hands(id, lowered_at), attendance(id)')
      .eq('institution_id', ap.institution_id).eq('status', 'active')
      .order('started_at', { ascending: false })
    setActiveSessions(active || [])
    const { data: recent } = await supabase
      .from('sessions')
      .select('id, started_at, ended_at, programs(name)')
      .eq('institution_id', ap.institution_id).eq('status', 'ended')
  ...
  async function startSession(programId) {
    const { data, error } = await supabase
      .from('sessions')
      .insert({ program_id: programId, institution_id: institution.id, status: 'active', started_at: new Date().toISOString() })
      .select().single()
  ...
  return (
    <AdminLayout>
      <div className="p-8 max-w-6xl mx-auto">
                ...
                return (
                  <div key={session.id} className="card flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-gray-900 truncate">{session.programs?.name}</p>
        ...
        {/* Programs */}
        <ProgramsPanel institutionId={institution?.id} onStart={startSession} />
                ...
                <tbody>
                  {recentSessions.map((s, i) => {
                    const duration = s.ended_at && s.started_at
                      ? Math.round((new Date(s.ended_at) - new Date(s.started_at)) / 60000) : null
                    return (
                      <tr key={s.id} className={`border-b border-gray-50 hover:bg-gray-50 cursor-pointer transition-colors ${i === recentSessions.length - 1 ? 'border-0' : ''}`}
                        ...
                        <td className="px-5 py-3 font-medium text-gray-900">{s.programs?.name}</td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {programs.map(program => (
            <div key={program.id} className="card flex items-center gap-4">
              ...
              <button onClick={() => onStart(program.id)} className="btn-primary text-sm py-2 px-4 flex-shrink-0">
                ▶ Start
              </button>
            </div>
          ))}
